...
//----------------------------------------------------------------------
ofxDropDown* ofxDropDown::setup(vector<string>& vals, int _maxTextLength, float width, float height) {
	ofxDropDown();
	list = vals;
...
//----------------------------------------------------------------------
ofxDropDown* ofxDropDown::setup(const string& dropDownName, vector<string>& vals, int _maxTextLength, float width, float height) {
	value.set(dropDownName, 0, 0, vals.size() - 1);
	return setup(vals, maxTextLength, width, height);
}
//----------------------------------------------------------------------
...
int ofxDropDown::generateBar(float my, bool bUseIndex) {
	...
	bar.rectangle(b.x, b.y + lineHeight*v, b.width - 2, lineHeight);
	return v;
}
//----------------------------------------------------------------------
void ofxDropDown::generateText() {
	...
	if (bGuiActive) {
		...
		std::string txt = "";
		if (!maxTextLength || list[value].size() <= maxTextLength)
			txt = list[value];
		else
			txt = list[value].substr(0, maxTextLength - 2) + "..";
		textMesh.append(getTextMesh(txt, b.x, b.y + b.height / 2 + 4));
	}
}
//----------------------------------------------------------------------
void ofxDropDown::buildListTextMeshes() {
	...
	for (int i = 0; i < list.size(); i++) {
		std::string txt = "";
		if (!maxTextLength || list[i].size() <= maxTextLength)
			txt = list[i];
		else
			txt = list[i].substr(0, maxTextLength - 2) + "..";
		fullMesh.append(getTextMesh(txt, b.x, lineHeight*i + b.y - offset.y + b.height / 2 + 4));
	}
}
//----------------------------------------------------------------------
